...
class ITaskAttributes(interface.Interface):
    ...
    status = schema.Choice(
        title = _(u'Status'),
        description = _(u'Select task status.'),
        vocabulary = 'project.task.status',
        required = False)
...
class ITaskAttributesConfig(interface.Interface):
    ...
    status = SimpleList(
        title = _(u'Status'),
        description = _(u'Ticket status.'),
        default = ['Work Queue', 'Waiting Data', 'In-Process','Internal Testing',
                   'Rework', 'Customer Testing', 'On-Hold', 'Rejected', 'Done'],
        required = False)
