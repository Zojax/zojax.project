...
from zojax.project.interfaces import _, ITaskAttributes, ITaskAttributesConfig
...
class TaskAttributesForm(PageletEditSubForm):
    ...
    def fields(self):
        ...
        voc = getUtility(IVocabularyFactory, 'project.task.status')(content)
        if not voc:
...
class TaskView(object):
    ...
    def update(self):
        ...
        # status
        voc = getUtility(IVocabularyFactory, 'project.task.status')(task)
        if voc:
            try:
                self.status = voc.getTerm(context.status).title
            except LookupError:
                self.status = u'---'
